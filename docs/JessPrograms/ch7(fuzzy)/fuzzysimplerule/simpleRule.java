...
//   extra code to show how to access the x,y pairs of a FuzzyValue
     FuzzyValue fvalsB[] = new FuzzyValue[3];
     fvalsB[0] = outFlow.findTerm("low");
     fvalsB[1] = outFlow.findTerm("OK");
     fvalsB[2] = outFlow.findTerm("strong");     
     System.out.println();
     int i, j;
     for (j=0; j<3; j++)
     {
     ...
     for (j=0; j<3; j++)
     {
       System.out.println("\nValues for term: " + fvalsB[j].getLinguisticExpression());
       for (i=0; i<fvalsB[0].size(); i++)
    	   System.out.println(fvalsB[j].getX(i) + ", " + fvalsB[j].getY(i));
     }
  }
